...
int push_timers(routing_plugin *o, struct timers t)
{
    ...
    char cmd[256];
    sprintf(cmd, "/config set olsrv2.tc_interval=%4.2f/config set interface.hello_interval=%4.2f/config commit/quit/exec=%4.6f algo/exec=%4.6f", t.tc_timer, t.h_timer, t.exec_time,t.algo_time);
